...
```
docker build -t secure-api:v2 .
docker images secure-api:v2
# secure-api:v2   235MB (content size 58.7MB)

docker history secure-api:v2 | grep -i gcc
# (пусто — компиляторы отсутствуют в финальных слоях)
```
